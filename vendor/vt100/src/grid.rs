    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..(count.min(self.size.rows - self.scroll_top)) {
            ...
            if self.scrollback_len > 0 {
                // Preserve lines scrolled out of pinned-layout UIs that use a
                // partial scroll region instead of the alternate screen.
...
mod tests {
    ...
    fn write_row(grid: &mut Grid, row: u16, text: &str) {
        for (col, ch) in text.chars().enumerate() {
            grid.drawing_cell_mut(Pos {
                row,
                col: col.try_into().unwrap(),
            })
            .unwrap()
            .set(ch, crate::attrs::Attrs::default());
        }
    }
    ...
    #[test]
    fn scroll_up_keeps_scrollback_when_scroll_region_is_active() {
        let mut grid = Grid::new(Size { rows: 4, cols: 4 }, 8);
        grid.allocate_rows();
        write_row(&mut grid, 0, "aaaa");
        write_row(&mut grid, 1, "bbbb");
        write_row(&mut grid, 2, "cccc");
        write_row(&mut grid, 3, "dddd");

        grid.set_scroll_region(1, 2);
        grid.scroll_up(1);

        assert_eq!(grid.scrollback.len(), 1);
        assert_eq!(row_text(grid.scrollback.front().unwrap(), 4), "bbbb");
        assert_eq!(row_text(grid.drawing_row(1).unwrap(), 4), "cccc");
    }
}
